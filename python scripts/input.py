...
# BOX PARAMETERS
target_thickness = 100 * um  # Thickness of the target
...
resx = 15  # Points per micron in x-direction, ho cambiato da 10 a 15
resy = 15  # Points per micron in y-direction
...
# TIME
Tsim = 1.2*Lx # Total simulation time
...
Z_H = 1  # Charge number of the ions (Carbon)
A_H = 1  # Atomic mass number of the ions (Carbon)
species_boundary_conditions = [['remove'], ['periodic']]  # Boundary conditions for species
n_target = 0.1  # Number density of the target in units of critical density
nppc_ion = 10  # Number of ions per cell
nppc_ele = 10  # Number of electrons per cell

# DIAGNOSTIC PARAMETERS
every_fs = int(fs / dt)  # Frequency of diagnostics in terms of femtoseconds
timescalar = [0, int(0.5 * every_fs)]  # Time stezps for scalar diagnostics
...
Main(
    ...
    interpolation_order=2, #ho cambiato da 2 a 4
    cell_length=[dx, dy],
    grid_length=[Lx, Ly],
...
Species(
    ...
    number_density=polygonal(xpoints=[targetstart, targetstart + target_thickness],
                             xvalues=[n_target / Z_H, n_target / Z_H]),
    mass=A_H * pemr,
    charge=Z_H,
