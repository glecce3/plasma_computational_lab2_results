...
Diag = S.ParticleBinning(1, units=unitsim)
...
for n, ts in enumerate(timesteps[::10]):
    ...
    plt.tight_layout()
    output_folder = os.path.expanduser('~/Desktop/output_spectra/electrons')
...
image_folder = os.path.expanduser('~/Desktop/output_spectra/electrons')
video_name = os.path.join(image_folder, 'output_video_spectra_electrons.mp4')
...
data = np.column_stack((E, Wele))  # Combina E e Wele in due colonne

# Salva in CSV con intestazione
np.savetxt('spectra_electrons.csv', data, delimiter=',', header='Eele,Wele', comments='')
