...
# BOX PARAMETERS
target_main = 1 * um  # Thickness of the target (C)
target_secondary = 0.15 * um # Thickness of the secondary target (H)
targetstart = 20 * um  # Start position of the target
aftertarget = 20 * um  # Distance after the target
Lx = targetstart + target_main + target_secondary + aftertarget  # Total length of the box along x-axis
Ly = 40 * um  # Total length of the box along y-axis
resx = 25  # Points per micron in x-direction
resy = 10  # Points per micron in x-direction
...
npatch_y = 8  # Number of patches in y-direction
...
# PLASMA PARAMETERS CARBON
Temp = 0.2 / mc2  # Temperature of the plasma in eV
Z_C = 6  # Charge number of the ions (Carbon)
A_C = 12  # Atomic mass number of the ions (Carbon)
species_boundary_conditions_C = [['remove'], ['periodic']]  # Boundary conditions for species
n_target_C = 40  # Number density of the target in units of critical density
nppc_ion_C = 10  # Number of ions per cell
nppc_ele = 30  # Number of electrons per cell

# PLASMA PARAMETERS HYDROGEN
Temp = 0.2 / mc2  # Temperature of the plasma in eV
Z_H = 1 # Charge number of the ions (Hydrogen)
A_H = 1  # Atomic mass number of the ions (Hydrogen)
species_boundary_conditions_H = [['remove'], ['periodic']]  # Boundary conditions for species
n_target_H = 10  # Number density of the target in units of critical density
nppc_ion_H = 60  # Number of ions per cell
nppc_ele = 60  # Number of electrons per cell
...
Species(
    ...
    particles_per_cell=nppc_ion_H,
    momentum_initialization="cold",
    number_density=polygonal(xpoints=[targetstart + target_main, targetstart + target_main + target_secondary],
                             xvalues=[n_target_H / Z_H, n_target_H / Z_H]),
    mass=A_H * pemr,
    charge=Z_H,
    boundary_conditions=species_boundary_conditions_H,
    pusher="boris"
)
...
Species(
    name="ion_C",
    position_initialization='random',
    particles_per_cell=nppc_ion_C,
    momentum_initialization="cold",
    number_density=polygonal(xpoints=[targetstart, targetstart + target_main],
                             xvalues=[n_target_C / Z_C, n_target_C / Z_C]),
    mass=A_C * pemr,
    charge=Z_C,
    boundary_conditions=species_boundary_conditions_C,
    pusher="boris"
)
...
Species(
    ...
    number_density=polygonal(xpoints=[targetstart, targetstart + target_main + target_secondary], 
                             xvalues=[n_target_H, n_target_H]),
    temperature=[Temp],
    mean_velocity=[0.],
    charge=-1.,
    mass=1.,
...
DiagFields(
    every=timefield,
    flush_every=1000,
    fields=["Rho_ele", "Rho_ion_H", "Rho_ion_C", "Bx", "By", "Bz", "Ex", "Ey", "Ez"]
)
